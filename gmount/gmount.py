...
def main():
    ...
    # Determine operation based on command name
    command_name = os.path.basename(sys.argv[0])
    is_umount = 'umount' in command_name

    # Parse command line arguments
    parser = argparse.ArgumentParser(
        description=f"{'Unmount' if is_umount else 'Mount'} filesystems"
    )
    parser.add_argument('-c', '--config', 
                        default=os.environ.get('GMOUNT_CONFIG', 'gmount.json'),
                        help='Path to JSON configuration file (default: GMOUNT_CONFIG env var or "gmount.json")')
    args = parser.parse_args()

    # Config file from flag (takes priority) or environment variable
    ...
    try:
        manager = MountManager(config_file, pass_manager)
        manager.load_config(user_vars=user_vars)
        success_count, failure_count = manager.process_mounts(user_vars, is_umount)
